...
struct tria_runtime {
    ...
    /* Cached env-var config (Task 7) — read once at init, never re-read */
    int     cfg_budget_tokens;      /* TRIA_BUDGET_TOKENS override, -1 = use pct */
    int     cfg_ramp_start_pct;     /* TRIA_RAMP_START_PCT, -1 = disabled */
    int     cfg_no_gpu_score;       /* 1 = force CPU (TRIA_NO_GPU_SCORE) */
    int     cfg_score_layer_stride; /* TRIA_SCORE_LAYER_STRIDE, default 1 */
    float   cfg_v_lambda;           /* TRIA_V_LAMBDA, default 0.25f */
    int     cfg_random;             /* TRIA_RANDOM mode, 0 or 1 */

    /* Persistent scratch buffers (Tasks 9+1) — reallocated only on growth */
    float  *scratch_k_f32;          /* [scratch_capacity * max_row_elems] */
    float  *scratch_scores;         /* [scratch_capacity] */
    int    *scratch_key_pos;        /* [scratch_capacity] */
    int    *scratch_phys;           /* [scratch_capacity] physical KV row indices (Task 1) */
    float  *scratch_k_real;         /* [scratch_capacity * max_fc] */
    float  *scratch_k_imag;         /* [scratch_capacity * max_fc] */
    int     scratch_capacity;       /* current row capacity */
    int     scratch_fc_capacity;    /* current fc capacity for k_real/k_imag */

    /* Cached llama_kv_cache* pointer (Task 10) — set by llama_context after init.
     * Avoids repeated dynamic_cast in bridge hot path. void* for C compatibility. */
    void * kv_cache_ptr;
};
...
int tria_get_kv_positions(void * ctx, int * positions, int * phys_indices, int max_positions);
...
#include <vector>
#include <cstdint>
int tria_compact_kv_with_positions(struct tria_runtime * rt, void * ctx_void, std::vector<uint32_t> & out_keep_positions);
#else
#endif
