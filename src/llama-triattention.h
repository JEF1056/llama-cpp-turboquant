...
// ============================================================================
// Binary calibration file format (upstream TRIA v1/v2 from triattention-ggml)
// ============================================================================
//
...
//   num_heads      uint32  e.g. 32  (attention query heads)
//   num_kv_heads   uint32  e.g. 8   (GQA KV heads)
//   head_dim       uint32  e.g. 128 (rotary_dim for partial-rotary models)
//   freq_count     uint32  head_dim / 2
//   rope_theta     float32 e.g. 1000000.0
//   attn_scale     float32 attention scaling metadata
//   reserved       28 bytes (zeros)
//
// v2 only — layer budget scales block (immediately after header):
//   layer_budget_scales  float32[num_layers]
//
// Per-head stats block (dense, layer-major order):
//   for layer in [0..num_layers-1]:
//     for head in [0..num_heads-1]:
//       q_mean_real   float32[freq_count]
//       q_mean_imag   float32[freq_count]
//       q_abs_mean    float32[freq_count]
//       mrl           float32[freq_count]  (skipped on load)
//
// v1 total size: 64 + num_layers * num_heads * freq_count * 16
// v2 total size: 64 + num_layers*4 + num_layers * num_heads * freq_count * 16
...
// Keep TRIATTENTION_VERSION for backward compat references elsewhere
...
// Model calibration data loaded from TRIA v1/v2 binary file
struct triattention_calibration {
    uint32_t head_dim;            // rotary_dim (may differ from model head_dim for partial-rotary)
    uint32_t num_layers;
    uint32_t num_attn_heads;      // total attention query heads
    uint32_t num_kv_heads;        // GQA KV heads
    uint32_t num_kv_groups;       // = num_attn_heads / num_kv_heads
    float    rope_theta;          // f32 as written by upstream calibrate.py
    uint32_t rope_style;          // 0 = half (Llama/Qwen), 1 = interleaved
    uint32_t freq_count;          // = head_dim / 2
    uint32_t n_sampled;           // = num_layers * num_attn_heads (dense)
    uint32_t file_version;        // 1 or 2

    // Per-layer budget scaling factors (v2: loaded from file; v1: all 1.0f)
    float * layer_budget_scales;  // [num_layers]

    // Per sampled head arrays — length n_sampled = num_layers * num_attn_heads
    // Filled densely in layer-major order: head[layer * num_attn_heads + head_idx]
