...
/* sincosf is a GNU extension not available on all platforms (e.g. macOS/MSVC).
 * Fall back to separate sinf/cosf when the GNU extension isn't declared. */
#if defined(__APPLE__) || !defined(_GNU_SOURCE)
static inline void tria_sincosf(float a, float *s, float *c) { *s = sinf(a); *c = cosf(a); }
#else
static inline void tria_sincosf(float a, float *s, float *c) { sincosf(a, s, c); }
#endif
...
struct tria_stats * tria_load(const char *path) {
    ...
    if (fread(&magic, 4, 1, fp) != 1 || fread(&version, 4, 1, fp) != 1) {
        LLAMA_LOG_WARN("tria_load: truncated header\n");
        fclose(fp); return NULL;
    }
    if (magic != TRIA_MAGIC || (version != 1 && version != 2 && version != 3)) {
        LLAMA_LOG_WARN("tria_load: bad magic/version: %x v%u\n", magic, version);
        fclose(fp); return NULL;
    }
    ...
    if (fread(&s->num_layers,   4, 1, fp) != 1 ||
        ...
        LLAMA_LOG_WARN("tria_load: truncated header fields\n");
    ...
    if (nl == 0 || nl > 1024 || nh == 0 || nh > 1024 || nkv == 0 || nkv > nh ||
        ...
        LLAMA_LOG_WARN("tria_load: invalid dimensions: nl=%u nh=%u nkv=%u fc=%u hd=%u\n",
                nl, nh, nkv, fc, s->head_dim);
        free(s); fclose(fp); return NULL;
    }
    /* Check for overflow: nl * nh */
    if (nl > UINT32_MAX / nh) {
        LLAMA_LOG_WARN("tria_load: overflow nl*nh\n");
        free(s); fclose(fp); return NULL;
    }
    ...
    if (version >= 2) {
        if (fread(s->layer_budget_scales, 4, nl, fp) != nl) {
            LLAMA_LOG_WARN("tria_load: truncated layer_budget_scales\n");
        ...
        if (bad) {
            LLAMA_LOG_WARN("tria_load: invalid layer_budget_scales, falling back to 1.0\n");
    ...
    for (uint32_t h = 0; h < total; h++) {
        ...
        if (!hs->q_mean_real || !hs->q_mean_imag || !hs->q_abs_mean || !hs->qma) {
            LLAMA_LOG_WARN("tria_load: alloc failed at head %u\n", h);
        ...
        if (fread(hs->q_mean_real, 4, fc, fp) != fc ||
            ...
            LLAMA_LOG_WARN("tria_load: truncated head data at %u\n", h);
            fclose(fp); tria_free(s); return NULL;
        }
        fseek(fp, fc * 4, SEEK_CUR);  /* skip mrl */
        if (ftell(fp) < 0) {
            LLAMA_LOG_WARN("tria_load: truncated mrl at head %u\n", h);
            fclose(fp); tria_free(s); return NULL;
        }
        ...
        if (version >= 3 && s->nonrot_dim > 0) {
            ...
            if (fread(hs->q_nonrot_mean, 4, nd, fp) != nd ||
                ...
                LLAMA_LOG_WARN("tria_load: truncated nonrot data at head %u\n", h);
                fclose(fp); tria_free(s); return NULL;
            }
        }
    }
    ...
    fclose(fp);
    LLAMA_LOG_INFO("tria_load: OK — %u layers, %u heads, %u kv_heads, fc=%u (v%u)\n",
...
static void tria_cs_free(struct tria_cs_table *t);  /* forward decl */
...
static struct tria_cs_table * tria_cs_precompute(
    ...
    /* Precompute per-offset cos/sin increments: O(TRIA_N_OFFSETS * fc) sincosf calls */
    float *off_cos = (float *)malloc((size_t)TRIA_N_OFFSETS * fc * sizeof(float));
    float *off_sin = (float *)malloc((size_t)TRIA_N_OFFSETS * fc * sizeof(float));
    if (!off_cos || !off_sin) {
        free(off_cos); free(off_sin);
        tria_cs_free(t);
        return NULL;
    }
    for (int o = 0; o < TRIA_N_OFFSETS; o++) {
        for (int f = 0; f < fc; f++) {
            tria_sincosf(offsets[o] * omega[f],
                         &off_sin[o * fc + f], &off_cos[o * fc + f]);
        }
    }

    /* Per-key: O(seq_len * fc) sincosf + O(seq_len * TRIA_N_OFFSETS * fc) multiply-adds */
    for (int s = 0; s < seq_len; s++) {
        ...
        for (int f = 0; f < fc; f++) {
            float base_cos, base_sin;
            tria_sincosf(base_delta * omega[f], &base_sin, &base_cos);
            for (int o = 0; o < TRIA_N_OFFSETS; o++) {
                int idx = (s * TRIA_N_OFFSETS + o) * fc + f;
                float oc = off_cos[o * fc + f];
                float os_v = off_sin[o * fc + f];
                /* cos(A+B) = cosA*cosB - sinA*sinB */
                t->cos_tab[idx] = base_cos * oc - base_sin * os_v;
                /* sin(A+B) = sinA*cosB + cosA*sinB */
                t->sin_tab[idx] = base_sin * oc + base_cos * os_v;
            }
        }
    }
...
static void score_keys_single_head(
    ...
    /* fc is validated ≤ TRIA_MAX_FC=1024 at load time, so 4KB max — safe for stack */
    if (tria_max_beta < 0.0f) {
        ...
        LLAMA_LOG_INFO("tria: max_beta=%.2f\n", tria_max_beta);
    }
    ...
    if (nonrot_alpha < 0.0f) {
        ...
        LLAMA_LOG_INFO("tria: nonrot_alpha=%.3f\n", nonrot_alpha);
    }
