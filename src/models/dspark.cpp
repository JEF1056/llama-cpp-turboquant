...
void llama_model_dspark::load_arch_hparams(llama_model_loader & ml) {
    ...
    // expose the tap layers through the shared llama_model_target_layer_ids()
    // getter (like eagle3) so a speculative driver can engage target capture
    target_layer_ids.clear();
    target_layer_ids.reserve(hparams.n_dspark_target_layers);
    for (uint32_t i = 0; i < hparams.n_dspark_target_layers; ++i) {
        target_layer_ids.push_back((int32_t) hparams.dspark_target_layers[i]);
    }
