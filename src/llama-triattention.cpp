...
// Free calibration arrays for h heads already allocated (used in error paths)
static void triattention_free_partial_heads(triattention_calibration * cal, uint32_t h_allocated) {
    for (uint32_t j = 0; j < h_allocated; j++) {
...
// Load upstream TRIA v1/v2 binary calibration file.
// Format written by triattention_calibrate.py (github.com/domvox/triattention-ggml).
...
static triattention_calibration * triattention_load_calibration(const char * path) {
    ...
    // ---- Read and validate magic + version (first 8 bytes) ----
    ...
    if (fread(&version, sizeof(uint32_t), 1, f) != 1 ||
        ...
        fprintf(stderr, "[TriAttention] ERROR: unsupported version %u in %s (supported: %u-%u)\n",
    ...
    cal->file_version = version;

    // ---- Read remaining fixed header fields (bytes 8..35) ----
    // Upstream header layout (64 bytes total, little-endian):
    //   [0]  magic       u32
    //   [4]  version     u32
    //   [8]  num_layers  u32
    //   [12] num_heads   u32
    //   [16] num_kv_heads u32
    //   [20] head_dim    u32   (= rotary_dim for partial-rotary models)
    //   [24] freq_count  u32   (= head_dim / 2)
    //   [28] rope_theta  f32
    //   [32] attn_scale  f32   (metadata only)
    //   [36] reserved    28 bytes
    float attn_scale_unused;
    bool ok = true;
    ...
    ok = ok && fread(&cal->rope_theta,     sizeof(float),    1, f) == 1;
    ok = ok && fread(&attn_scale_unused,   sizeof(float),    1, f) == 1;
    ...
    // Seek to byte 64 (skip reserved bytes) regardless of struct packing
    ...
    if (cal->num_layers == 0 || cal->head_dim == 0) {
        fprintf(stderr, "[TriAttention] ERROR: invalid dimensions (layers=%u, head_dim=%u) in %s\n",
                cal->num_layers, cal->head_dim, path);
        delete cal;
        fclose(f);
        return nullptr;
    }
    ...
    // ---- v2: read per-layer budget scales; v1: synthesize 1.0f ----
    ...
    if (version >= 2) {
        ...
        for (uint32_t i = 0; i < cal->num_layers; i++) {
            cal->layer_budget_scales[i] = 1.0f;
        }
    }

    // ---- Read dense per-head stats (layer-major order) ----
    // n_sampled = num_layers * num_attn_heads (all entries, including zero-filled SSM layers)
    cal->n_sampled = cal->num_layers * cal->num_attn_heads;
    ...
    std::vector<float> skip_buf(fc);  // scratch for mrl (4th array, skipped)

    for (uint32_t li = 0; li < cal->num_layers; li++) {
        for (uint32_t hi = 0; hi < cal->num_attn_heads; hi++) {
            const uint32_t h = li * cal->num_attn_heads + hi;
            cal->sampled_layer[h] = li;
            cal->sampled_head[h]  = hi;
            ...
            ok  = fread(hs.q_mean_real, sizeof(float), fc, f) == fc;
            ok  = ok && fread(hs.q_mean_imag, sizeof(float), fc, f) == fc;
            ok  = ok && fread(hs.q_abs_mean,  sizeof(float), fc, f) == fc;
            // Skip mrl (4th array — used only by calibration tool)
            ok  = ok && fread(skip_buf.data(), sizeof(float), fc, f) == fc;

            if (!ok) {
                fprintf(stderr, "[TriAttention] ERROR: truncated stats at layer=%u head=%u in %s\n",
                        li, hi, path);
                // h arrays allocated including this one; h+1 heads total with this one partially done
                delete[] hs.q_mean_real;
                delete[] hs.q_mean_imag;
                delete[] hs.q_abs_mean;
                hs.q_mean_real = hs.q_mean_imag = hs.q_abs_mean = nullptr;
                triattention_free_partial_heads(cal, h);
                fclose(f);
                return nullptr;
            }
        }
    ...
    fprintf(stderr, "[TriAttention] Loaded TRIA v%u calibration: layers=%u, attn_heads=%u, "
            "kv_heads=%u, head_dim=%u (freq=%u), rope_theta=%.1f%s\n",
            version, cal->num_layers, cal->num_attn_heads,
            cal->num_kv_heads, cal->head_dim, cal->freq_count, cal->rope_theta,
            version >= 2 ? " [per-layer budget scales loaded]" : "");
    ...
    uint32_t yarn_n_ctx_orig)
{
    ...
    // NOTE: upstream calibrate.py stores rotary_dim (not full head_dim) in the head_dim field
    // for partial-rotary models (e.g. Qwen3.5 with partial_rotary_factor < 1.0).
    // For standard models (partial_rotary_factor = 1.0), head_dim == rotary_dim.
    // We accept calibration head_dim <= model head_dim to handle partial-rotary.
    if (cal->head_dim > head_dim) {
        fprintf(stderr, "[TriAttention] ERROR: calibration head_dim (%u) > model head_dim (%u) in %s\n",
                cal->head_dim, head_dim, stats_path);
        triattention_free_calibration(cal);
        return nullptr;
    }
    if (cal->head_dim != head_dim) {
        fprintf(stderr, "[TriAttention] INFO: calibration head_dim=%u (rotary_dim) < model head_dim=%u "
    ...
    if (fabsf(cal->rope_theta - (float)rope_theta) / fmaxf(cal->rope_theta, 1.0f) > 0.01f) {
        fprintf(stderr, "[TriAttention] WARNING: rope_theta mismatch (calibration=%.1f, model=%.1f)\n",
                (double)cal->rope_theta, rope_theta);
    }
    ...
    uint32_t              kv_size)
{
    ...
    if (cfg.mode == TRIATTENTION_MODE_GLOBAL) {
        ...
        // v2: multiply each head's score by its layer's budget scale so that
        // layers with higher scales (more diffuse attention, lower MRL) contribute
        // proportionally higher scores → their tokens are preferentially retained.
        for (uint32_t i = 0; i < n_decode; i++) {
            float max_score = -1e30f;
            for (uint32_t sh = 0; sh < cal->n_sampled; sh++) {
                const uint32_t layer_idx = cal->sampled_layer[sh];
                const float scale = cal->layer_budget_scales[layer_idx];  // 1.0f for v1
                float s = score_buf[sh * n_decode + i] * scale;
        ...
        for (uint32_t kv_h = 0; kv_h < n_kv; kv_h++) {
            ...
            for (uint32_t i = 0; i < n_decode; i++) {
                ...
                for (uint32_t sh_idx : heads) {
                    const uint32_t layer_idx = cal->sampled_layer[sh_idx];
                    const float scale = cal->layer_budget_scales[layer_idx];  // 1.0f for v1
                    float s = score_buf[sh_idx * n_decode + i] * scale;
        ...
        if (cfg.normalize_scores) {
            ...
            // Recompute combined after normalization (with layer scale)
            for (uint32_t i = 0; i < n_decode; i++) combined[i] = -1e30f;
            for (uint32_t sh = 0; sh < cal->n_sampled; sh++) {
        ...
        // Aggregate: scale-weighted mean score across all sampled heads for each token.
        // v2: layer_budget_scales weights each layer's contribution.
        // v1: all scales are 1.0f → identical to unweighted mean.
        for (uint32_t i = 0; i < n_decode; i++) {
            float sum = 0.0f;
            float weight_sum = 0.0f;
            for (uint32_t sh = 0; sh < cal->n_sampled; sh++) {
                const float scale = cal->layer_budget_scales[cal->sampled_layer[sh]];
                sum += score_buf[sh * n_decode + i] * scale;
                weight_sum += scale;
            }
            combined[i] = (weight_sum > 0.0f) ? sum / weight_sum : 0.0f;
        }
    ...
    if (cfg.enable_logging) {
        // First prune: confirm triattention is active
        if (state->total_prune_calls == 1) {
            fprintf(stderr, "[TriAttention] First prune at pos=%lld: budget=%u, window=%u [%s]\n",
                    (long long)state->absolute_position,
                    cfg.budget, cfg.divide_length,
                    state->use_gpu ? "GPU scoring" : "CPU scoring");
        }

        // Per-prune event log
        uint32_t n_kept = n_occupied - n_evicted;
        float kept_pct  = n_occupied > 0 ? 100.0f * n_kept  / n_occupied : 0.0f;
        float evict_pct = n_occupied > 0 ? 100.0f * n_evicted / n_occupied : 0.0f;
        fprintf(stderr, "[TriAttention] prune #%llu  pos=%-8lld  %u→%u (-%u/%.0f%%)  "
                "protected=%u  %.1f ms\n",
                (unsigned long long)state->total_prune_calls,
                (long long)state->absolute_position,
                n_occupied, n_kept, n_evicted, evict_pct,
                n_protected,
                state->last_prune_time_ms);
        (void)kept_pct; // suppress unused-variable warning if optimized out

        // Periodic summary every 50 prunes
        if (state->total_prune_calls % 50 == 0) {
            double avg_ms = state->total_prune_time_ms / (double)state->total_prune_calls;
            fprintf(stderr, "[TriAttention] --- summary after %llu prunes: "
                    "%llu total evicted, avg %.1f ms/prune ---\n",
                    (unsigned long long)state->total_prune_calls,
...
void triattention_print_stats(const triattention_state * state, FILE * stream) {
    ...
    fprintf(stream, "  Calibration ver:  v%u\n", state->cal->file_version);
