...
common_params_context common_params_parser_init(common_params & params, llama_example ex, void(*print_usage)(int, char **)) {
    ...
    add_opt(common_arg(
        {"--kv-autosave-interval"}, "SECONDS",
        string_format("periodically save idle KV cache to --slot-save-path every N seconds (default: %d; -1 = disabled)", params.kv_autosave_interval),
        [](common_params & params, int value) {
            if (value == 0 || value < -1) {
                throw std::invalid_argument("invalid value: cannot be 0 or less than -1");
            }
            params.kv_autosave_interval = value;
        }
    ).set_examples({LLAMA_EXAMPLE_SERVER}));
    add_opt(common_arg(
        {"--kv-cache-expiry"}, "SECONDS",
        string_format("expire stored KV cache (*.llama_cache) files older than N seconds; they are\n"
                      "skipped and deleted on server load and during each --kv-autosave-interval pass\n"
                      "(default: %d; -1 = disabled)", params.kvc_cache_expiry),
        [](common_params & params, int value) {
            if (value == 0 || value < -1) {
                throw std::invalid_argument("invalid value: cannot be 0 or less than -1");
            }
            params.kvc_cache_expiry = value;
        }
    ).set_examples({LLAMA_EXAMPLE_SERVER}));
    add_opt(common_arg(
        {"--simple-io"},
