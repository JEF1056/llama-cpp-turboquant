...
size_t mtmd_input_chunk_serialize(const mtmd_input_chunk * chunk, uint8_t ** out_buf) {
    if (!chunk || !out_buf) return 0;
    if (chunk->type != MTMD_INPUT_CHUNK_TYPE_IMAGE && chunk->type != MTMD_INPUT_CHUNK_TYPE_AUDIO) {
        return 0; // text chunks are not serialized here
    }

    const std::string id = (chunk->type == MTMD_INPUT_CHUNK_TYPE_IMAGE)
        ? chunk->tokens_image->id
        : chunk->tokens_audio->id;
    const uint32_t n_tokens = (uint32_t)mtmd_input_chunk_get_n_tokens(chunk);
    const uint32_t id_len   = (uint32_t)id.size();

    // calculate buffer size: type(1) + n_tokens(4) + id_len(4) + id(id_len)
    //                        + [IMAGE: nx(4) + ny(4) + pos_type(1) + image_idx(4)]
    const bool is_image = (chunk->type == MTMD_INPUT_CHUNK_TYPE_IMAGE);
    const size_t buf_size = 1 + 4 + 4 + id_len + (is_image ? 13 : 0);

    uint8_t * buf = (uint8_t *)malloc(buf_size);
    if (!buf) return 0;

    size_t off = 0;
    auto write_u8  = [&](uint8_t  v) { buf[off++] = v; };
    auto write_u32 = [&](uint32_t v) { memcpy(buf + off, &v, 4); off += 4; };

    write_u8((uint8_t)chunk->type);
    write_u32(n_tokens);
    write_u32(id_len);
    if (id_len > 0) {
        memcpy(buf + off, id.data(), id_len);
        off += id_len;
    }
    if (is_image) {
        write_u32(chunk->tokens_image->nx);
        write_u32(chunk->tokens_image->ny);
        write_u8((uint8_t)chunk->tokens_image->pos);
        write_u32(chunk->tokens_image->image_idx);
    }
...
mtmd_input_chunk * mtmd_input_chunk_deserialize(const uint8_t * buf, size_t buf_size) {
    if (!buf || buf_size < 9) return nullptr; // minimum: type(1)+n_tokens(4)+id_len(4)

    size_t off = 0;
    auto read_u8  = [&](uint8_t  & v) -> bool { if (off + 1 > buf_size) return false; v = buf[off++]; return true; };
    auto read_u32 = [&](uint32_t & v) -> bool { if (off + 4 > buf_size) return false; memcpy(&v, buf + off, 4); off += 4; return true; };

    uint8_t type_u8 = 0;
    uint32_t n_tokens = 0, id_len = 0;
    if (!read_u8(type_u8) || !read_u32(n_tokens) || !read_u32(id_len)) return nullptr;

    const auto type = (mtmd_input_chunk_type)type_u8;
    if (type != MTMD_INPUT_CHUNK_TYPE_IMAGE && type != MTMD_INPUT_CHUNK_TYPE_AUDIO) return nullptr;

    if (off + id_len > buf_size) return nullptr;
    std::string id(reinterpret_cast<const char *>(buf + off), id_len);
    off += id_len;

    auto * chunk = new mtmd_input_chunk{ type, {}, nullptr, nullptr };

    if (type == MTMD_INPUT_CHUNK_TYPE_IMAGE) {
        if (off + 13 > buf_size) { delete chunk; return nullptr; }
        uint32_t nx = 0, ny = 0, image_idx = 0;
        uint8_t pos_u8 = 0;
        if (!read_u32(nx) || !read_u32(ny) || !read_u8(pos_u8) || !read_u32(image_idx)) { delete chunk; return nullptr; }
        auto * img = new mtmd_image_tokens();
        img->nx        = nx;
        img->ny        = ny;
        img->pos       = (mtmd_pos_type)pos_u8;
        img->image_idx = image_idx;
        img->id        = id;
        // batch_f32 left empty — KV tensors were saved by llama_state_seq_save_file
        chunk->tokens_image = mtmd_image_tokens_ptr(img);
    } else {
        auto * aud = new mtmd_audio_tokens();
        aud->n_tokens = n_tokens;
        aud->id       = id;
        chunk->tokens_audio = mtmd_audio_tokens_ptr(aud);
    }
